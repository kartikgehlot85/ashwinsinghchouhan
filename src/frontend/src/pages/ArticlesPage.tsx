...
export function ArticlesPage() {
  const { data: items = [], isLoading } = useContent("article");
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      >
        <div className="flex items-center gap-4">
          <div className="p-3 rounded-xl bg-secondary/20 border border-secondary/40">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Insights and perspectives on pharmacology & medicine
            </p>
          </div>
        </div>
        {isAdmin && (
      ...
      {addOpen && (
        <ContentFormModal type="article" onClose={() => setAddOpen(false)} />
      )}
      {editItem && (
        <ContentFormModal
          type="article"
